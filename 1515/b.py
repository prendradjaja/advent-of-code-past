...
def main():
    ...
    ingredients = []
    for line in lines:
        ingredients.append(tuple(findints(line)))

    answer = max(score(amounts) for amounts in get_amounts() if is_500_calories(amounts))
    print(answer)


def get_totals(amounts):
...
def score(amounts):
    totals = get_totals(amounts)
    if any(value <= 0 for value in totals):
        return 0
    else:
        return math.prod(
            totals[:-1]  # Last item is calories: Remove it
        )


def is_500_calories(amounts):
    totals = get_totals(amounts)
    return totals[-1] == 500
