...
def main():
    ...
    while True:
        tick(track, carts)
        if len(carts) == 1:
            print(carts[0].get_xy_position())
            exit()


def tick(track, carts):
    '''
    Mutates items in carts AND carts itself
    '''
    carts_to_iterate = sorted(carts, key=lambda cart: cart.position)
    removed = []
    for cart in carts_to_iterate:
        if cart in removed:
            continue
        cart.move()
