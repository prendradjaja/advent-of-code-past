...
def main():
    ...
    print('n', 'first', 'val', sep='\t')
    print(0, '-', get_total_resource_value(grid), sep='\t')

    first_seen = {str(grid): 0}
    ...
    for n in range(1, 1000):
        ...
        gstr = str(grid)
        if gstr in first_seen:
            gnum = first_seen[gstr]
        else:
            gnum = '-'
            first_seen[gstr] = n

        print(n, gnum, get_total_resource_value(grid), sep='\t')
