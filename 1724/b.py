...
def main():
    ...
    length = get_longest(components)
    get_all(components, length)
...
def get_longest(components):
    ...
    try:
        while True:
            i += 1
            longest = max(longest, len(make_bridge(components, indices)))
            if i % 1000 == 0:
                print(i // 1000, '?a', indices)
            ...
            if is_valid(components, deepen(indices)):
                ...
                while not is_valid(components, indices):
                    indices = carry_increment(indices)
                    if indices == ():
                        print('that one case i shouldve removed got hit (a)')
                        return
    except IndexError:
        return longest

def get_all(components, length):
    indices = (0,)
    i = 0
    best = 0
    try:
        while True:
            ...
            if len(bridge) == length:
                best = max(best, strength(bridge))
            if i % 1000 == 0:
                print(i // 1000, '?b', indices)
            # print(make_bridge(components, indices))
            if is_valid(components, deepen(indices)):
                indices = deepen(indices)
            elif is_valid(components, increment(indices)):
                ...
                while not is_valid(components, indices):
                    indices = carry_increment(indices)
                    if indices == ():
                        print('that one case i shouldve removed got hit (b)')
                        return
    except IndexError:
        print(best)
